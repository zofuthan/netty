...
public class DnsQueryTest {
    ...
    public void writeQueryTest() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ByteBuf packet = (ByteBuf) embedder.readOutbound();
            ByteBuf expected = embedder.alloc().buffer(512);
            DnsQueryEncoder.encodeQuery(query, expected);
            Assert.assertEquals("Malformed packet", packet, expected);
        }
    }
