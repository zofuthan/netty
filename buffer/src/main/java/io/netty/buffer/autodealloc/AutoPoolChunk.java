...
final class AutoPoolChunk<T> {
    ...
    final AutoPoolArena<T> arena;
    final T memory;
    final boolean unpooled;

    private final int[] memoryMap;
    private final AutoPoolSubpage<T>[] subpages;
    ...
    AutoPoolChunkList<T> parent;
    AutoPoolChunk<T> prev;
    AutoPoolChunk<T> next;

    // TODO: Test if adding padding helps under contention
    //private long pad0, pad1, pad2, pad3, pad4, pad5, pad6, pad7;

    AutoPoolChunk(AutoPoolArena<T> arena, T memory, int pageSize, int maxOrder, int pageShifts, int chunkSize) {
        unpooled = false;
        this.arena = arena;
        this.memory = memory;
    ...
    AutoPoolChunk(AutoPoolArena<T> arena, T memory, int size) {
        unpooled = true;
        this.arena = arena;
        this.memory = memory;
        memoryMap = null;
    ...
    private AutoPoolSubpage<T>[] newSubpageArray(int size) {
        return new AutoPoolSubpage[size];
    }
    ...
    private long allocateSubpage(int normCapacity, int curIdx, int val) {
        ...
        if (state == ST_ALLOCATED_SUBPAGE) {
            AutoPoolSubpage<T> subpage = subpages[subpageIdx(curIdx)];
    ...
    private long allocateSubpageSimple(int normCapacity, int curIdx, int val) {
        ...
        for (;;) {
            if (runLength == pageSize) {
                ...
                AutoPoolSubpage<T> subpage = subpages[subpageIdx];
                if (subpage == null) {
                    subpage = new AutoPoolSubpage<T>(this, curIdx, runOffset(val), pageSize, normCapacity);
    ...
    void free(long handle) {
        ...
        if (state == ST_ALLOCATED_SUBPAGE) {
            assert bitmapIdx != 0;
            AutoPoolSubpage<T> subpage = subpages[subpageIdx(memoryMapIdx)];
    ...
    void initBuf(AutoPooledByteBuf<T> buf, long handle, int reqCapacity) {
        int memoryMapIdx = (int) handle;
        int bitmapIdx = (int) (handle >>> 32);
        if (bitmapIdx == 0) {
            int val = memoryMap[memoryMapIdx];
    ...
    void initBufWithSubpage(AutoPooledByteBuf<T> buf, long handle, int reqCapacity) {
        initBufWithSubpage(buf, handle, (int) (handle >>> 32), reqCapacity);
    }

    private void initBufWithSubpage(AutoPooledByteBuf<T> buf, long handle, int bitmapIdx, int reqCapacity) {
        assert bitmapIdx != 0;
