...
final class AutoPooledUnsafeDirectByteBuf extends AutoPooledByteBuf<ByteBuffer> {

    private static final boolean NATIVE_ORDER = ByteOrder.nativeOrder() == ByteOrder.BIG_ENDIAN;
    private long memoryAddress;

    AutoPooledUnsafeDirectByteBuf(int maxCapacity) {
        super(maxCapacity);
    }

    @Override
    void init(AutoPoolChunk<ByteBuffer> chunk, long handle, int offset, int length, int maxLength) {
        super.init(chunk, handle, offset, length, maxLength);
        initMemoryAddress();
    }

    @Override
    void initUnpooled(AutoPoolChunk<ByteBuffer> chunk, int length) {
    ...
    public ByteBuf getBytes(int index, ByteBuf dst, int dstIndex, int length) {
        ...
        if (length != 0) {
            if (dst instanceof AutoPooledUnsafeDirectByteBuf) {
                AutoPooledUnsafeDirectByteBuf bbdst = (AutoPooledUnsafeDirectByteBuf) dst;
    ...
    public ByteBuf setBytes(int index, ByteBuf src, int srcIndex, int length) {
        checkIndex(index, length);
        if (length != 0) {
            if (src instanceof AutoPooledUnsafeDirectByteBuf) {
                AutoPooledUnsafeDirectByteBuf bbsrc = (AutoPooledUnsafeDirectByteBuf) src;
    ...
    public ByteBuf copy(int index, int length) {
        checkIndex(index, length);
        AutoPooledUnsafeDirectByteBuf copy =
                (AutoPooledUnsafeDirectByteBuf) alloc().directBuffer(capacity(), maxCapacity());
        if (length != 0) {
