 ...
 */
class AcceptedServerChannelRequestDispatch extends ChannelInboundMessageHandlerAdapter<FullHttpRequest> {
    ...
    private void handleOpenTunnel(ChannelHandlerContext ctx) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("open tunnel request received from " +
                    ctx.channel().remoteAddress() + " - allocated ID " +
                    tunnelId);
        }
        respondWith(ctx,
    ...
    private void handleSendData(ChannelHandlerContext ctx, FullHttpRequest request) {
        String tunnelId = checkTunnelId(request, ctx);
        if (tunnelId == null) {
            return;
        }
        ...
        if (HttpHeaders.getContentLength(request, 0) == 0 ||
                request.data().readableBytes() == 0) {
            respondWithRejection(ctx, request,
                    "Send data requests must contain data");
            return;
        }

        messageSwitch.routeInboundData(tunnelId, request.data());
     ...
     * for this operation.
     */
    private static ChannelFuture respondWith(ChannelHandlerContext ctx,
